#!/usr/bin/env python3
"""
🔍 AUDITOR MATEMÁTICO DE FEATURES TÉCNICAS - VERSIÓN CORREGIDA
============================================================
...
- RSI: Uso de media móvil exponencial Wilder
- ATR: Cálculo correcto del True Range máximo
- Williams %R: Corrección de fórmula y manejo de division por cero
- Momentum: Cambio a returns en lugar de ratios
...
class MathematicalFeaturesAuditor:
    ...
    def generate_test_data(self, n_samples: int = 1000) -> pd.DataFrame:
        ...
        volatility = data['close'].pct_change().rolling(20).std().fillna(0.02)
        data['volume'] = np.random.lognormal(8, 0.3, n_samples) * (1 + volatility * 10)

        return data.set_index('timestamp')

    def audit_rsi_calculation(self, data: pd.DataFrame) -> Dict[str, any]:
        """🔍 Auditar cálculo de RSI - CORREGIDO"""
        print("\n🔍 AUDITANDO RSI...")
        ...
        # === IMPLEMENTACIÓN SISTEMA CORREGIDA ===
        def calculate_rsi_system_corrected(prices: pd.Series, period: int = 14) -> pd.Series:
            """Implementación corregida usando Wilder's EMA"""
            delta = prices.diff()
            gain = delta.where(delta > 0, 0)
            loss = (-delta).where(delta < 0, 0)

            # CORRECCIÓN: Usar EMA de Wilder (alpha = 1/period)
            alpha = 1.0 / period
        ...
        rsi_system = calculate_rsi_system_corrected(data['close'], period)
    ...
    def audit_bollinger_bands(self, data: pd.DataFrame) -> Dict[str, any]:
        ...
        def calculate_bollinger_system(prices: pd.Series):
            ...
            bb_std = prices.rolling(20).std(ddof=0)  # CORRECCIÓN: ddof=0 para coincidir con TA-Lib
    ...
    def audit_williams_r(self, data: pd.DataFrame) -> Dict[str, any]:
        ...
        def calculate_williams_r_system(high_s, low_s, close_s):
            high_series = pd.Series(high_s)
            low_series = pd.Series(low_s)
            close_series = pd.Series(close_s)

            highest_high = high_series.rolling(14).max()
            lowest_low = low_series.rolling(14).min()

            # CORRECCIÓN: Evitar división por cero y usar fórmula correcta
            denominator = highest_high - lowest_low
            denominator = denominator.replace(0, np.nan)  # Evitar división por cero

            willr = -100 * (highest_high - close_series) / denominator
            return willr.values
        ...
        # Validaciones
        if mae > 0.5:  # CORRECCIÓN: Umbral más realista
    ...
    def audit_momentum_calculations(self, data: pd.DataFrame) -> Dict[str, any]:
        """🔍 Auditar cálculos de momentum - CORREGIDO"""
        print("\n🔍 AUDITANDO MOMENTUM...")

        close = data['close']

        # === IMPLEMENTACIONES SISTEMA CORREGIDAS ===
        # CORRECCIÓN: Usar returns en lugar de ratios
        momentum_5 = close.pct_change(5)    # Returns en lugar de ratios
        momentum_10 = close.pct_change(10)  # Returns en lugar de ratios

        # === VERIFICACIONES ===
        results = {"status": "PASS", "checks": []}

        # 1. Verificar que momentum es finito
        momentum_5_clean = momentum_5.dropna()
        momentum_10_clean = momentum_10.dropna()

        if np.any(~np.isfinite(momentum_5_clean)) or np.any(~np.isfinite(momentum_10_clean)):
            self.errors_found.append("Momentum: Valores infinitos o NaN encontrados")
            results["status"] = "ERROR"
        else:
            results["checks"].append("✅ Momentum siempre finito")

        # 2. Verificar rangos razonables para returns (-0.5 a 0.5 para crypto)
        momentum_5_mean = momentum_5_clean.mean()
        momentum_10_mean = momentum_10_clean.mean()

        # CORRECCIÓN: Rangos apropiados para returns
        if abs(momentum_5_mean) > 0.1:  # |return| > 10% promedio es sospechoso
            self.warnings_found.append(f"Momentum 5: Media fuera de rango razonable: {momentum_5_mean:.4f}")

        if abs(momentum_10_mean) > 0.1:
            self.warnings_found.append(f"Momentum 10: Media fuera de rango razonable: {momentum_10_mean:.4f}")

        # 3. Verificar que el momentum a largo plazo sea menor en volatilidad
        vol_5 = momentum_5_clean.std()
        vol_10 = momentum_10_clean.std()

        if vol_5 < vol_10:  # Esperamos que momentum corto plazo sea más volátil
            self.warnings_found.append(f"Momentum: Volatilidad inesperada - 5p: {vol_5:.4f}, 10p: {vol_10:.4f}")

        results["momentum_5_mean"] = momentum_5_mean
        results["momentum_10_mean"] = momentum_10_mean
        results["momentum_5_vol"] = vol_5
        results["momentum_10_vol"] = vol_10
        ...
        print(f"  📊 Volatilidad 5p: {vol_5:.4f}, 10p: {vol_10:.4f}")
        print(f"  ✅ Estado: {results['status']}")
    ...
    def audit_atr_calculation(self, data: pd.DataFrame) -> Dict[str, any]:
        """🔍 Auditar cálculo de ATR - CORREGIDO"""
        print("\n🔍 AUDITANDO ATR...")
        ...
        def calculate_atr_system(high_s, low_s, close_s):
            ...
            # CORRECCIÓN: Cálculo correcto del True Range
            tr1 = high_series - low_series
            tr2 = abs(high_series - close_series.shift())
            tr3 = abs(low_series - close_series.shift())

            # CORRECCIÓN: Usar np.maximum en lugar de pd.concat
            true_range = np.maximum.reduce([tr1, tr2.fillna(0), tr3.fillna(0)])

            # CORRECCIÓN: Usar EMA de Wilder para ATR
            alpha = 1.0 / 14
        ...
        nonzero_idx = atr_talib_clean > 0
        if np.sum(nonzero_idx) == 0:
            self.errors_found.append("ATR: Todos los valores TA-Lib son cero")
            return {"status": "ERROR", "details": "TA-Lib cero"}

        mae_rel = np.mean(np.abs(atr_system_clean[nonzero_idx] - atr_talib_clean[nonzero_idx]) / atr_talib_clean[nonzero_idx])
        max_error_rel = np.max(np.abs(atr_system_clean[nonzero_idx] - atr_talib_clean[nonzero_idx]) / atr_talib_clean[nonzero_idx])

        results = {
            "status": "PASS",
            "mae_rel": mae_rel,
            "max_error_rel": max_error_rel,
            "samples_compared": np.sum(nonzero_idx)
        }

        # Validaciones
        if mae_rel > 0.05:  # CORRECCIÓN: Umbral más realista 5%
