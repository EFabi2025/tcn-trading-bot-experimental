...
class AdvancedFeatureEngineer:
    ...
    def select_best_features(self, features: pd.DataFrame, top_n: int = 25) -> pd.DataFrame:
        """Seleccionar las mejores features"""
        if features.empty:
            return features
        
        # Features esenciales que siempre incluir
        essential_features = [
        ...
        available_essential = [f for f in essential_features if f in features.columns]
        
        # Calcular importancia simple (varianza normalizada)
        feature_importance = {}
        for col in features.columns:
            if col not in available_essential:
                # Métrica simple: combinación de varianza y autocorrelación
                variance_score = features[col].var()
                autocorr_score = features[col].autocorr(lag=1) if len(features) > 1 else 0
                feature_importance[col] = variance_score * (1 - abs(autocorr_score))
        
        # Seleccionar top features adicionales
        sorted_features = sorted(feature_importance.items(), key=lambda x: x[1], reverse=True)
        additional_features = [f[0] for f in sorted_features[:top_n - len(available_essential)]]
        
        selected_features = available_essential + additional_features
...
class EnhancedTCNPredictor:
    ...
    def _create_enhanced_model(self):
        """Crear modelo mejorado optimizado"""
        model = tf.keras.Sequential([
            # Entrada optimizada
            tf.keras.layers.LSTM(128, return_sequences=True, input_shape=(60, 25)),
    ...
    async def predict_enhanced(self, pair: str, market_data: dict) -> dict:
        ...
        features = self.feature_engineer.select_best_features(features, top_n=25)
        ...
        # Crear secuencia optimizada (60 timesteps)
        sequence_length = 60
        if len(features_scaled) < sequence_length:
            return None
        
        sequence = features_scaled[-sequence_length:, :25]  # Top 25 features
